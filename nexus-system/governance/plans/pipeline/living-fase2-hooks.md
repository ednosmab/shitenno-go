# Pipeline: Nexus Living — Fase 2 (Git Hooks Reactivos)
...
2. Fazer commit — o hook `post-commit` deve correr `nexus detect --auto`
3. Verificar que o plano foi arquivado para `governance/plans/done/`
4. Correr `nexus hooks install` — deve fazer append ao `post-commit` existente
...
|---|---|---|---|---|
| C1 | detect --auto funciona | Correr `nexus detect --auto` | Zero erro de opção desconhecida | pendente |
| C2 | checkAndArchiveDonePlans é idempotente | Correr 2x seguidas sobre mesmo estado | Sem duplicação, sem falha | pendente |
| C3 | Instalador nunca sobrescreve hook | Correr `nexus hooks install` com post-commit preenchido | Conteúdo original preservado | pendente |
...
npx vitest run src/__tests__/plan-lifecycle.test.ts src/__tests__/hooks-install.test.ts \
  ...
  && nexus pipeline notify living-fase2-hooks --event "phase_complete"
```
