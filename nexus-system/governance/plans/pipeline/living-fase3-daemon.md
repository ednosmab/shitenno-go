# Pipeline: Nexus Living — Fase 3 (Daemon Opt-In)
...
1. Correr `nexus daemon start` — daemon deve arrancar e ficar activo
2. Correr `nexus daemon status` — deve mostrar PID e estado "running"
3. Correr `nexus status` — deve responder via socket IPC (mais rápido)
4. Correr `nexus daemon stop` — daemon deve encerrar limpo
5. Verificar que ficheiros temporários (PID, socket) foram removidos
6. Correr `nexus daemon start` 3x seguidas com crash forçado — circuit breaker deve desistir
7. Correr `nexus daemon start` com `NEXUS_NO_DAEMON=1` — não deve arrancar daemon
8. Correr `nexus daemon start` com `CI=true` — não deve arrancar daemon
...
4. `stat -c %a nexus-system/daemon/daemon.sock` — confirmar permissão 0600
5. `grep -n "chmod.*0600" src/daemon.ts` — confirmar chmod no código
6. `grep -n "NEXUS_NO_DAEMON" src/daemon-client.ts` — confirmar detecção de env var
...
|---|---|---|---|---|
| C1 | Daemon arranca | `nexus daemon start` | PID gravado, estado "running" | pendente |
| C2 | Daemon responde | `nexus daemon status` | Output com PID e uptime | pendente |
...
| C6 | NEXUS_NO_DAEMON=1 | Correr com env var | Nenhum daemon spawnado | pendente |
...
npx vitest run src/__tests__/daemon.test.ts src/__tests__/daemon-client.test.ts \
  ...
  && nexus pipeline notify living-fase3-daemon --event "phase_complete"
```
