...
**Status:** In Progress
**Updated_at:** 2026-07-13T01:40:56.611Z
**Date:** 2026-07-12
...
```typescript
const lifecycle = detectLifecycleState(projectRoot, nexusDir);  // linha 548
// ...
...
`NexusLifecycleState` (`src/nexus-state-machine.ts`) são:
...
function calculateEntropy(
  ...
  relations: Relation[],
  lifecycle: NexusLifecycleState // ← NOVO parâmetro — já existe em escopo no caller
...
- Para o Item B: correr `nexus status` num projecto real em fase `governed`/`evolved`
