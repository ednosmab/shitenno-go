...
- [ ] Bug 1 — criar `src/feedback-utils.ts`
- [ ] Bug 2 — adicionar `banner()` em `src/formatting.ts`
- [ ] Bug 3 — corrigir os 5 `RULE-0{11..15}.json`
- [ ] Bug 4 — corrigir os 2 `plugin.js`
- [ ] Bug 5 — corrigir `nexus-system/answers.json` **e** blindar `fillPlaceholders`
- [ ] Rodar `npm run build && npx vitest run && npx tsc --noEmit` — todos devem passar limpos
- [ ] Rodar `npx tsx bin/nexus.ts init --dir <projeto-vazio> --answers-file nexus-system/answers.json` como teste manual de fumaça
- [ ] Registrar este ciclo em `docs/history/` (o próprio projeto já tem esse costume — ver `nexus-relatorio-bugs-correcoes.md`), fechando o loop de "Every Decision Generates Knowledge" (Princípio 5)
