...
│   │   ├── project.ts         # ProjectProfile, NexusState
...
|---|---|---|---|
| 1.1 | Criar `src/domain/entities/` e extrair tipos de `NexusState`, `ProjectProfile` de `engineering-state.ts` e `scorer.ts` | Novo: `project.ts`, `maturity.ts`, `feedback.ts`, `plan.ts`, `knowledge.ts`, `rule.ts` | `grep -r "NexusState" src/` — todos importam de `domain/entities/project.ts` |
