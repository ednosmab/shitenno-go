...
### Step 1.2: Corrigir duration no bin/nexus.ts

**Ficheiro:** `bin/nexus.ts`
...
**Verificação:** Após `nexus status`, `sessions.jsonl` tem `duration > 0`
...
- Adicionar `"command.completed"` ao `NexusEventType`
...
**Verificação:** `nexus init` gera eventos em `events-*.jsonl`
...
**Verificação:** `nexus run` gera múltiplos eventos específicos
...
- `trigger_assessment`: Executar `nexus assess` via `execSync` com `NEXUS_CHILD=1`
- `trigger_health_check`: Executar `nexus doctor` via `execSync` com `NEXUS_CHILD=1`

### Step 4.3: Criar 5 regras reais

**Directório:** `nexus-system/governance/rules/`
...
- Importar e chamar `initializeTaskPipeline({ projectRoot, nexusDir })`
- Guardar função de cleanup

### Step 6.2: Validar integração

- `nexus validate` → `validation.completed` → task pipeline processa
...
**Directório:** `nexus-system/plugins/health-monitor/plugin.js`
...
**Directório:** `nexus-system/plugins/event-logger/plugin.js`

- Hook `custom-metric`: registar métricas de cada comando
- Output: `nexus-system/telemetry/plugin-metrics.jsonl`
...
1. `nexus init` → eventos publicados
2. `nexus status` → `analysis.complete` + `command.completed`
3. `nexus audit` → `health.checked` + regras executadas
4. `nexus validate` → `validation.completed` + task pipeline
...
| `bin/nexus.ts` | 1,5,6 | Duration, watcher cleanup, task pipeline init |
...
| `nexus-system/governance/rules/` | 4 | 5 regras novas |
| `nexus-system/plugins/` | 7 | 2 plugins |
