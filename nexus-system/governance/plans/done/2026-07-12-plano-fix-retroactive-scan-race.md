...
> **Contexto:** achado da validação de Fase 1/2 do Nexus Living. `initPlanBacklogSync()`
> corre em todo bootstrap de comando (`bin/nexus.ts:123`), só é saltado quando
> `NEXUS_CHILD` está definido (subprocessos do próprio nexus) — não protege contra
> duas invocações **independentes** do CLI a correr ao mesmo tempo (ex: `post-commit`
> dispara `nexus detect --auto &` em background enquanto o utilizador corre outro
...
As duas camadas são complementares: o lock evita colisão de escrita; o cooldown evita que o scan sequer tente correr a cada `nexus status`, `nexus doctor`, etc., o que já reduz drasticamente a janela de colisão e o custo (readdir + import dinâmico a cada comando).

Alternativa considerada e **rejeitada**: restringir o scan só a `nexus watch`. Resolveria a concorrência de raiz, mas perde a auto-cura para quem não usa `watch` — é uma mudança de comportamento maior do que o bug pede. Fica como opção de recuo se o lock/cooldown se mostrar insuficiente em produção.
...
function shouldSkipScan(nexusDir: string): boolean {
...
function markScanRun(nexusDir: string): void {
...
3. **Integração (recomendado, prioridade menor)**: usar `child_process.spawn` para
   lançar dois processos `nexus` reais contra o mesmo `nexusDir` fixture quase em
...
- Teste manual: correr `nexus status` duas vezes em rápida sucessão num projecto
