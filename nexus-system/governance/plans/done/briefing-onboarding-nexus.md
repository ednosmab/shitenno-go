# Briefing: Onboarding do Nexus System (README + Dashboard)

> Documento de trabalho para o agente responsável pela implementação do Nexus System.
...
O Nexus System tem uma base técnica sólida (93 módulos, 500+ testes, um dashboard funcional que já lê dados reais do projeto). O problema não é capacidade — é **sequenciamento de informação**.

Hoje, tanto o README quanto as páginas `discover/` e `use/` do dashboard abrem com conceito antes de mostrar uso. Uma pessoa que nunca ouviu falar do Nexus precisa entender "Knowledge Debt", "Engineering State" e o "Meta Model" antes de rodar o primeiro comando. Isso empurra a pessoa pra fora antes dela ver qualquer valor prático.

Sintoma concreto: o dashboard já busca dados reais do projeto (`fingerprint.json`, `maturity-profile.json`, `context_buffer.yaml`) — isso é a parte que funciona bem. Mas as páginas de entrada (`discover/WhatIsNexus`, `discover/Why`, `use/Installation`, `use/FirstSteps`) são texto estático com o mesmo vocabulário filosófico do README, só que dividido em abas. A interface melhorou, o conteúdo não.
...
Critério de aceite: **teste dos 5 minutos**. Pegue alguém sem contexto, dê o link do README ou o dashboard, cronometre até essa pessoa rodar `nexus init` e entender o output sem te perguntar nada. Se passar de 5 minutos até o primeiro comando executado com sucesso, a página falhou.
...
## 5. Escopo 2 — Dashboard (`apps/nexus-dashboard`)
...
|---|---|---|---|
| 1 | `WhatIsNexus` | O que isso faz? | Reescrever abertura sem jargão; adicionar 1 print/GIF do dashboard mostrando dados reais de um projeto |
...
| 4 | `WhyNexus` | Por que isso existe (aprofundamento)? | Mover para depois do "começar" — é justificativa, não é pré-requisito |
...
Criar uma página nova (`use/TeamOnboarding` ou seção dentro de `WhoIsItFor`) respondendo especificamente: **"Eu entrei num time que já usa Nexus, o que eu faço no primeiro dia?"**. Conteúdo mínimo:
...
3. Um comando único de "me dê o resumo do estado atual" (provavelmente `nexus status` ou o briefing cacheado) que substitui a pergunta "alguém pode me atualizar sobre o projeto?".

Essa página resolve diretamente a lacuna que identificamos: o Nexus promete resolver dor de onboarding para os *usuários dele*, mas hoje não aplica isso a si mesmo.
