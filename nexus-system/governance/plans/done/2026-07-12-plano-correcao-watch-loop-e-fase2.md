...
- [x] `nexus init` chama o instalador automaticamente — testar em projeto novo: `.git/hooks/post-commit`
...
- [x] Atualizar `nexus-system/docs/BACKLOG.md` — mudar `LIVING-002` de `Status: Backlog` para

## BUG-002 (CRÍTICO, bloqueante) — `nexus watch` entra em loop de eventos

### Sintoma
Ao editar um arquivo real de governança com `nexus watch` rodando, o terminal entra em rajadas repetidas
...
2. `sync-docs.ts` escreve `nexus-system/docs/generated/ARCHITECTURE.md`.
...
```bash
grep -n "writeFileSync\|mkdirSync" src/*.ts nexus-system/scripts/*.ts | grep -i "sync-docs\|generated"
```
Qualquer outro diretório que `sync-docs.ts` (ou processos parecidos) escrevam **dentro** de `nexus-system/docs/`
ou `nexus-system/governance/` precisa da mesma entrada `0.0` na tabela, pelo mesmo motivo.
...
describe("docs/generated/ exclusion (BUG-002 regression)", () => {
  it("scores files inside docs/generated/ as zero significance", () => {
    const score = detectDirectoryScore(`${NEXUS}/docs/generated/ARCHITECTURE.md`, NEXUS);
...
it("does not re-trigger docs.sync.triggered when syncing writes to docs/generated/", async () => {
  ...
  const generatedPath = join(nexusDir, "docs", "generated", "ARCHITECTURE.md");
...
| `nexus init` instala hooks automaticamente | ❌ Testado em projeto novo — nenhum hook é criado |
...
**Ligar em `src/commands/init.ts`** — chamar `installReactiveHooks(projectRoot, nexusBinPath)` no passo
...
export function scheduledCheck(projectRoot: string, nexusDir: string): void {
...
Adicionar novo comando interno `nexus internal-scheduled-check` (não documentado no `--help` principal,
só pra uso via cron) que chama `scheduledCheck()`. Instalar entrada de cron opcional no `init` (perguntar,
não forçar):
```bash
*/15 * * * * cd <projectRoot> && node <nexusBinPath> internal-scheduled-check >> nexus-system/daemon/scheduled.log 2>&1
```

**Nota:** adicionar `"workdir.large_uncommitted_drift"` ao union type `NexusEventType` em `event-bus.ts`
...
- [x] `installReactiveHooks()` implementado, testado (5 testes acima passando).
- [x] `nexus init` chama o instalador automaticamente — testar em projeto novo: `.git/hooks/post-commit`
  (ou `.husky/post-commit` se aplicável) contém o marcador `nexus-managed-hook` depois do `init`.
- [x] `scheduledCheck()` implementado e testado (simular drift alto, confirmar evento publicado; simular
