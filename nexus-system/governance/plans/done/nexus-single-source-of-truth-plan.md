# Plano: Engineering State como Única Fonte de Verdade — Nexus System

**Status:** Done

## Contexto

Auditoria end-to-end do Nexus System (build + typecheck + 1679 testes passando + execução real de `init`, `status`, `run`, `audit`, `doctor` em um projeto TypeScript novo) revelou que, apesar da base filosófica e arquitetural estar bem projetada (event sourcing, capability lifecycle, knowledge graph, ADRs), **a implementação ainda viola o próprio princípio central do sistema**: "Engineering State é a fonte única de verdade" (ADR-002).
...
```ts
const maturityProfile = loadMaturityProfile(nexusDir);          // valor persistido/oficial
const installedCapabilities = detectInstalledCapabilities(nexusDir); // heurística de filesystem
...
**Sintoma:** `nexus init`/`status` reportam `Stack: none detected` mesmo com `typescript` + `tsconfig.json` presentes.
...
**Sintoma:** projeto recém-criado via `nexus init` já sai com 3 itens críticos e score 1/100.
...
**Sintoma:** `nexus doctor` mostra Health 85/100 e 1 capacidade instalada; `nexus audit` mostra Score 1/100; `nexus status` mostra 7 capacidades instaladas. O arquivo persistido (`maturity-profile.json`) diz `installedCapabilities: ["core"]`.

### Bug 5 — Pipeline reporta estágio pulado como sucesso
**Sintoma:** `nexus run` imprime `⚠ Skipping evolve stage (requires 'governed' state)` e, duas linhas depois, `✔ evolve (0ms)` / "5 stage(s) succeeded".
...
export function consolidateEngineeringState(
  ...
  const fsDetected = detectCapabilitySignalsFromFilesystem(nexusDir); // ver Fase 3 (rename)
...
`capabilityDrift` deve virar um achado de primeira classe em `nexus audit`: se `detectedNotRegistered` não estiver vazio, reportar como item de governança ("capacidade parece configurada no disco mas não registrada — rode `nexus sync`"), em vez de cada comando decidir sozinho em qual fonte confiar.
...
const context: RuleContext = {
  ...
  installedCapabilities: maturityProfile.installedCapabilities, // era detectInstalledCapabilities(nexusDir)
};
```
...
describe("Engineering State é fonte única de verdade", () => {
  it("status, doctor e audit reportam o mesmo health score e as mesmas capacidades", async () => {
    const { projectRoot, nexusDir } = await createFixtureProject(); // helper já existente na suíte

    const state = getEngineeringState(projectRoot, nexusDir, true);
  ...
  it("nexus run nunca reporta um estágio pulado como sucesso", async () => {
