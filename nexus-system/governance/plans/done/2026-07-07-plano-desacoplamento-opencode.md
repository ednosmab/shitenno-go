...
| 2 | `src/nexus-state-machine.ts` | `detectLifecycleState` — removida a mesma checagem redundante de `opencode.json`. Parâmetro `projectRoot` ficou sem uso; prefixado `_projectRoot`. |
| 3 | `src/analyser.ts` | Campo `hasNexus` corrigido pra checar `nexus-system/` de verdade. |
...
|---|---|---|
| `src/__tests__/cli-integration.test.ts` | Exigia `nexus-system/` sozinho = "not initialized" | Reescrito: `nexus-system/` sozinho = inicializado |
...
- Ponta a ponta, projeto novo, `opencode.json` removido: `status`, `audit`, `goal create`, `nexus mcp` — todos funcionando.
...
- Formato Cursor usa `nexusMcp` (camelCase); `.mcp.json` usa `nexus-mcp` (kebab-case).
...
Teste actualizado. Entrada `sync: "governed"` manteve-se porque sync foi restaurado em `bin/nexus.ts`.

### B.4 `src/commands/sync.ts` — Decisão tomada ✅

Sync foi restaurado em `bin/nexus.ts:267`. Comando funcional, gate `sync: "governed"` válido.
...
- Helper `ensureContextBuffer(nexusDir)` extraído para eliminar duplicação

### B.8 Event Bus — 10 de 34 eventos declarados nunca são publicados

**Diferido.** Rebaixado de prioridade porque orquestração multi-agente não é objectivo do Nexus (ver Parte C). Backlog de qualidade interna.
