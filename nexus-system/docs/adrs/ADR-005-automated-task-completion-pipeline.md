...
This revealed a systemic gap: there are **15 state management systems** in the Nexus codebase (plan, step, goal, action, backlog, session, document lifecycle, capability, asset, etc.), but most lack automated transition logic. The backlog in particular has **8 documented states** (AGENTS.md Rule #16) but **0 implemented in code** — items only ever transition between "Backlog" and "Done" via manual edits.
...
- `nexus-system/scripts/close-session.ts` — session closure validation
...
- `nexus-system/docs/BACKLOG.md` — backlog file with 8-state vocabulary in AGENTS.md
