# Plano: Production Readiness — nexus-cli

**Data:** 2026-06-27
**Objetivo:** Cobrir os gaps restantes para o nexus ser um sistema production-ready.
...
### 1.3 `src/scaffolder.ts:91` — `scaffoldNexusSystem`
 ...
 *
 * @param nexusDir - Diretório do nexus-system
...
describe("Edge Cases", () => {
  ...
  it("handles project with 1000+ files", async () => {
    ...
    const tmpDir = "/tmp/nexus-bulk-test";
    // ... setup ...
    const analysis = analyseProject(tmpDir);
    const start = Date.now();
    await calculateComplexityScore(tmpDir, `${tmpDir}/nexus-system`, analysis);
