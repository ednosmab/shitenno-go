# Nexus System — Relatório de Execução E2E + Correções

> Gerado após instalar, buildar, testar e executar o pacote `nexus-system`
...
## 🐛 BUG 1 — RESOLVIDO: Regex corrompe `nexus-profile/*.config.ts`
...
Em `src/__tests__/scaffolder.test.ts`, novo bloco `describe("generated nexus-profile/*.config.ts content")` com 5 testes que leem o **conteúdo real** do arquivo gerado (não só verificam se ele existe):

```ts
import { mkdtempSync, rmSync, mkdirSync, existsSync, readFileSync } from "node:fs";
// ... (adicionar readFileSync ao import existente)

describe("generated nexus-profile/*.config.ts content", () => {
  ...
  function readGeneratedProfile(dir: string): string {
    ...
    const profilePath = join(dir, "nexus-profile", `${projectName}.config.ts`);
...
## 🐛 BUG 2 — ABERTO: `nexus run` ignora o lifecycle gate que `nexus evolve` respeita
...
No mesmo estado do projeto (`assessed`), chamar `nexus evolve` diretamente é bloqueado pela state machine, mas o estágio `evolve` dentro de `nexus run` executa sem checagem.
...
if (!checkLifecycleGate("evolve", ctx.projectRoot, ctx.nexusDir, true)) {
...
**Opção B — Documentar que `run` é uma "via expressa" intencional**, que avança o estado conforme necessário em vez de bloquear (mais alinhado com "Nexus propõe, humano decide": rodar `run` é uma ação humana explícita, então pode ser tratado como consentimento implícito para rodar tudo). Se for essa a intenção, adicionar um comentário explícito no código justificando, para não parecer um descuido da próxima vez que alguém ler.
...
## 🐛 BUG 3 — ABERTO: `nexus report` mostra a mesma dimensão como força E fraqueza
...
Isso é especialmente importante porque **todo projeto novo começa com dimensões empatadas** — não é um edge case raro, é o estado inicial padrão de qualquer adoção do Nexus.
...
## ⚠️ NÃO-BUG, mas observação relevante: `nexus init` é 100% interativo, sem flag não-interativa

`nexus init` faz 9 blocos de perguntas via `inquirer` (tipos `input`, `checkbox`, `list`, `confirm`), sem nenhuma flag tipo `--yes` ou `--config <file>` para rodar de forma não-interativa/scriptável. Isso é aceitável para uso manual direto pelo Tech Lead, mas significa que **não é possível automatizar a inicialização do Nexus a partir de outro script ou CI** sem ou (a) adicionar essa flag, ou (b) chamar `scaffoldNexusSystem` programaticamente (que é o que usei para testar nesta sessão, contornando o prompt).

Se o objetivo for permitir, por exemplo, que um template de projeto já venha com Nexus pré-configurado automaticamente, vale considerar adicionar:

```bash
nexus init --answers-file ./nexus-answers.json
```
...
- `scaffoldNexusSystem` (pós-correção) gera 24 arquivos / 10 pastas corretamente
- `nexus status` produz health check + complexity breakdown por área, granular e legível
- `nexus assess` recalcula o perfil de maturidade e avança o estado da máquina corretamente
- `nexus doctor` dá a saída mais útil de todas — risco, melhoria, e uma seção "Learn" explicando conceito (ADR vs Skill) de forma natural, sem parecer enfeite
- `nexus report` reusa genuinamente as 7 dimensões do `feedback/TEMPLATE.md` original do course-platform — a herança conceitual é real, não só similar de nome
