docs/
└── evolution/
    ├── README.md                        ← índice mestre
    │
    ├── 00-EXECUTIVE-SUMMARY.md
    ├── 01-CURRENT-STATE-ASSESSMENT.md
    ├── 03-TARGET-ARCHITECTURE.md
    ├── 26-ARCHITECTURE-INVARIANTS.md
    │
    ├── domain/
    │   ├── 04-DOMAIN-REFINEMENT.md
    │   ├── 05-BOUNDED-CONTEXTS.md
    │   ├── 06-APPLICATION-LAYER.md
    │   └── 07-USE-CASES.md
    │
    ├── platform/
    │   ├── 08-DOMAIN-CATALOG.md
    │   ├── 09-CAPABILITY-EVOLUTION.md
    │   ├── 10-PLUGIN-SYSTEM.md
    │   ├── 11-PIPELINE.md
    │   ├── 12-STATE-MACHINE.md
    │   ├── 13-KNOWLEDGE-GRAPH.md
    │   └── 14-TELEMETRY.md
    │
    ├── quality/
    │   ├── 15-BUG-FIX-PLAN.md
    │   ├── 16-TESTING-STRATEGY.md
    │   ├── 17-PERFORMANCE.md
    │   ├── 18-SECURITY.md
    │   ├── 19-DOCUMENTATION.md
    │   └── 20-ARCHITECTURE-GOVERNANCE.md
    │
    ├── roadmap/
    │   ├── 21-MIGRATION-WAVE-1.md
    │   ├── 22-MIGRATION-WAVE-2.md
    │   ├── 23-MIGRATION-WAVE-3.md
    │   ├── 24-MIGRATION-WAVE-4.md
    │   └── 25-LONG-TERM-ROADMAP.md
    │
    └── ai/
        ├── MASTER_EVOLUTION_PLAN.md
        ├── AI-IMPLEMENTATION-GUIDE.md
        ├── TASK-CATALOG.md
        ├── EXECUTION-ORDER.md
        ├── VALIDATION-CHECKLIST.md
        ├── ROLLBACK-PLAN.md
        └── ACCEPTANCE-CRITERIA.md