...
The process of adding a capability to a project via `nexus upgrade --capability <name>`.
...
### State Machine
A finite state machine governing Nexus's own lifecycle: `uninitialized → discovered → assessed → governed → evolved → mature`.
...
### Plugin System
An extensibility mechanism allowing projects to add custom checks, recommendations, and capabilities without modifying Nexus core.
...
The absolute principle: Nexus proposes changes, never auto-applies them. Humans maintain final authority.
