# Pipeline: Shugo Living — Fase 1 (Cache Cross-Process)
...
1. Correr `shugo status` — deve responder sem erro e mostrar health score
2. Correr `shugo doctor` — deve mostrar recomendações
3. Correr `shugo audit` — deve listar assets do projecto
4. Aguardar 70 segundos (cache expira após 60s)
5. Correr `shugo status` novamente — deve usar cache do disco (mais rápido)
6. Modificar um ficheiro em `governance/` (ex: adicionar linha a um .md)
7. Correr `shugo status` — deve recalcular (cache obsoleto detectado)
8. Correr `shugo status --force-refresh` — deve recalcular sempre, ignorando cache
...
| C5 | shugo status funciona | Correr `shugo status` | Output sem erro, health score visível | pendente |
| C6 | shugo doctor funciona | Correr `shugo doctor` | Output sem erro, recomendações visíveis | pendente |
...
npx vitest run src/__tests__/engineering-state-access.test.ts \
  ...
  && shugo pipeline notify living-fase1-cache --event "phase_complete"
```
