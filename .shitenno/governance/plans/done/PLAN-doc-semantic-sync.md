...
| `shitenno-go/scripts/sync-docs.ts` | Roda no `pre-commit`. Faz checagem **estrutural** (README vs comandos reais, versão CHANGELOG vs package.json). Gera `shitenno-go/reports/doc-sync-{date}.json`. **Não escreve em `context_buffer.yaml`.** |
...
export function runSemanticDocSync(options: SemanticSyncOptions): SemanticSyncResult {
  const { projectRoot, shitenDir, docsDir = join(projectRoot, "docs") } = options;
...
No final do `shitenno-go/scripts/sync-docs.ts`, dentro da função `main()` (ou equivalente, próximo de `generateReport()`), adicionar:
...
- **Cooldown implícito pelo dedup**, mas não há expiração — um reminder não resolvido fica para sempre até `clearRemindersByCategory` ser chamado (o agente deveria chamar isso depois de escrever a doc). Vale adicionar isso como follow-up: o agente, ao resolver um reminder de categoria `docs`, chama `clearRemindersByCategory(shitenDir, "docs")` no fim da tarefa.
