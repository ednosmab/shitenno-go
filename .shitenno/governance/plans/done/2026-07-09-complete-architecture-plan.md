# Plano Completo: Shitenno-go — Reactive + Proactive + Histórico + Agentes IA
...
Transformar o Shiten num sistema reactivo, proactivo, com histórico consultável e interface para agentes de IA — eliminando código morto, unificando health scores, e completando a visão de "amanhã" sem orquestração multi-agente.
...
├─────────────────────────────────────────────────────────────────┤
│                     AGENTES IA (Fase C)                         │
│  shiten context --for-agent, seleção de modelo, schema estável  │
├─────────────────────────────────────────────────────────────────┤
...
### Passo 1.2: Integrar no bootstrap `bin/shiten.ts`
...
export function initializeProactiveEngine(
  ...
  const onStateConsolidated = async () => {
    const state = consolidateEngineeringState(projectRoot, shitenDir);
    const capResult = evaluateCapabilities(state, shitenDir);
    const recResult = runRecommendationEngine(state, capResult, shitenDir);
...
### Passo 3.2: Integrar no bootstrap `bin/shiten.ts`
...
export function consolidateState(projectRoot: string, shitenDir: string): ShitenState {
  // ... existing code ...
}
```
...
export function saveEngineeringState(shitenDir: string, state: EngineeringState): void {
  ...
  const latestPath = join(shitenDir, "engineering-state.json");
...
```typescript
export function getSnapshotAt(shitenDir: string, timestamp: string): EngineeringState | null;
export function listSnapshots(shitenDir: string, range?: { from: string; to: string }): SnapshotMeta[];
...
```
shiten history [--from <date>] [--to <date>] [--diff]
```

### Passo A.5: Criar teste `src/__tests__/engineering-state-history.test.ts`

**Validação:** `shiten history --diff` mostra exactamente o que mudou
...
Ler de `answers.json` (já coletado no `shiten init`).

### Passo C.3: Criar teste `src/__tests__/context-command.test.ts`

**Validação:** `shiten context --for-agent` é determinístico
...
- Chamar `registerDocSyncHook()` no `bin/shiten.ts` bootstrap
...
| `bin/shiten.ts` | 1,3,E | +3 imports, +3 chamadas |
...
| **Agentes IA** | Nenhuma | `shiten context --for-agent` |
...
shiten history --diff mostra causa (não só efeito)
shiten context --for-agent é determinístico e não requer reanálise
