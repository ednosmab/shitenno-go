...
inclusive `shiten --help`, porque o import falha no carregamento do módulo.
...
describe("banner", () => {
  ...
  it("works without a subtitle", () => {
    ...
    banner("shiten init");
    const output = spy.mock.calls.map((c) => c[0]).join("\n");
    expect(output).toContain("shiten init");
    spy.mockRestore();
  });
});
```
...
📁 `shitenno-go/governance/rules/RULE-012.json`
```json
{
  "id": "RULE-012",
...
📁 `shitenno-go/governance/rules/RULE-013.json`
```json
{
  "id": "RULE-013",
...
📁 `shitenno-go/governance/rules/RULE-014.json`
```json
{
  "id": "RULE-014",
...
📁 `shitenno-go/governance/rules/RULE-015.json`
```json
{
  "id": "RULE-015",
...
// Sugestão: contar falhas e expor no resultado de shiten audit/doctor
logger.warn("RuleEngine", `Failed to parse ${file} — rule ignored, JSON inválido`);
invalidRuleFiles.push(file); // acumular e reportar em shiten audit
```
Isso torna o problema visível em `shiten audit` (que é exatamente o comando cujo propósito é
...
### Correção — `shitenno-go/plugins/event-logger/plugin.js`
...
### Correção — `shitenno-go/plugins/health-monitor/plugin.js`
...
## Bug 5 — `shiten init --answers-file` crasha com o `answers.json` bundlado
...
```
Causa raiz: `shitenno-go/answers.json` (o exemplo shippado no próprio repositório) só tem o bloco
...
Isso afeta diretamente o primeiro comando do Quick Start do README (`shiten init`), no modo não
interativo — o caminho que qualquer automação/CI usaria.

### Correção A — consertar o arquivo de exemplo `shitenno-go/answers.json`
...
const baseMaturity = {
  usedShitenBefore: true, isFirstProject: false, projectAge: "established",
...
describe("scaffoldShitennoGo with minimal answers file", () => {
  it("does not throw when stack/database/styling are provided", () => {
    ...
    expect(() => scaffoldShitennoGo("/tmp/shiten-answers-test", answers, ["core"])).not.toThrow();
  });
});
```
...
- [x] Bug 5 — corrigir `shitenno-go/answers.json` **e** blindar `fillPlaceholders`
- [x] Rodar `npm run build && npx vitest run && npx tsc --noEmit` — todos devem passar limpos
- [x] Rodar `npx tsx bin/shiten.ts init --dir <projeto-vazio> --answers-file shitenno-go/answers.json` como teste manual de fumaça
- [x] Registrar este ciclo em `docs/history/` (o próprio projeto já tem esse costume — ver `shiten-relatorio-bugs-correcoes.md`), fechando o loop de "Every Decision Generates Knowledge" (Princípio 5)
