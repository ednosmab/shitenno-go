...
The Shitenno-go has 25+ rules in AGENTS.md, but not all rules are relevant for every project. A simple CLI tool doesn't need TDD, session invariants, or quick board rules. The system should adapt its complexity to the project's complexity.
...
**File:** `shitenno-go/docs/REGISTRY.md` (new)
...
**File:** `shitenno-cli/src/complexity-detector.ts` (new)
...
**File:** `shitenno-cli/src/rule-loader.ts` (new)
...
**File:** `shitenno-go/docs/AGENTS.md`
...
**File:** `shitenno-cli/src/commands/assess.ts` (modify)

Add complexity detection subcommand:

```bash
shiten assess complexity
# Output:
...
**File:** `shitenno-cli/src/commands/briefing.ts` (modify)
