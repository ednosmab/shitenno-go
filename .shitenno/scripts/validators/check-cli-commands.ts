...
export function checkCLICommands(ctx: ValidatorContext) {
  ...
  const documented = implemented.filter((cmd) => guide.includes(`shugo ${cmd}`));
  const undocumented = implemented.filter((cmd) => !guide.includes(`shugo ${cmd}`));

  pass(ctx, `${documented.length}/${implemented.length} commands documented in README`);

  for (const cmd of undocumented) {
    warn(ctx, `Command implemented but not documented: shugo ${cmd}`, undefined, false);
  }
