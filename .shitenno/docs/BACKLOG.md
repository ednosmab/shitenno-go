...
| **Owner** | executor |
| **Data** | 2026-06-30 |
| **Fonte** | shugo assess --json (dimension: governance = 15 → 100) |
...
| **Owner** | executor |
| **Data** | 2026-07-11 |
| **Fonte** | Plano Shugo Living v2 (secção 3, Fase 1) |
...
### BUG-001 Corrigir `shugo detect --auto`
...
| **Owner** | executor |
| **Data** | 2026-07-11 |
| **Fonte** | Plano Shugo Living v2 (secção 0, achado novo) |
| **Modulos** | src/commands/detect.ts, .husky/post-commit |
| **Descricao** | `.husky/post-commit` executa `shugo detect --auto 2>/dev/null &` mas `detect.ts` não define opção `--auto`. Commander.js rejeita opção desconhecida. Hook falha silenciosamente a cada commit (stderr redirigido para /dev/null). |
...
| **Owner** | unassigned |
| **Data** | 2026-06-30 |
| **Fonte** | shugo assess --json (dimension: architecture = 15) |
| **Modulos** | src/ (global) |
...
| **Owner** | executor |
| **Data** | 2026-06-30 |
| **Fonte** | shugo assess --json (dimension: documentation = 10) |
| **Modulos** | src/, docs/ |
...
| **Owner** | unassigned |
| **Data** | 2026-06-30 |
| **Fonte** | shugo audit --json (knowledgeGraph.suggestions) |
| **Modulos** | shitenno/ |
...
| **Owner** | executor |
| **Data** | 2026-06-30 |
| **Fonte** | shugo status --json (context_buffer.yaml = warn) |
| **Modulos** | shitenno/governance/context/ |
...
| **Owner** | executor |
| **Data** | 2026-06-30 |
| **Fonte** | shugo status --json (agent contracts = warn) |
| **Modulos** | shitenno/governance/agents/ |
...
| **Owner** | unassigned |
| **Data** | 2026-07-11 |
| **Fonte** | Plano Shugo Living v2 (secção 3, Fase 2) |
...
| **Owner** | unassigned |
| **Data** | 2026-07-11 |
| **Fonte** | Plano Shugo Living v2 (secção 3, Fase 3) |
| **Depende** | LIVING-001 + LIVING-002 (validação alguns dias) |
| **Modulos** | src/daemon.ts (NOVO), src/daemon-client.ts (NOVO), src/daemon-circuit-breaker.ts (NOVO), src/commands/daemon.ts (NOVO), src/daemon-resources.ts (NOVO), src/cli-middleware.ts, src/commands/init.ts, bin/shugo.ts |
| **Descricao** | Daemon opt-in com socket IPC (chmod 0600), handshake de versão, circuit breaker de crash loop, `SHITENNO_NO_DAEMON=1` + `CI=true`, auto-start só após validação manual. Reaproveita `checkAndArchiveDonePlans()` da Fase 2 em modo contínuo via chokidar. |
| **Correcao** | 5 ajustes de segurança obrigatórios. Estrutura: daemon.pid, daemon.sock, daemon.log, daemon.rules.json em `shitenno/daemon/`. |
...
| **Owner** | unassigned |
| **Data** | 2026-07-11 |
| **Fonte** | Plano Shugo Living v2 (secção 5) |
...
| **Owner** | unassigned |
| **Data** | 2026-07-11 |
| **Fonte** | Plano Shugo Living v2 (secção 4) |
...
| **Owner** | executor |
| **Data** | 2026-07-11 |
| **Fonte** | Shugo Living — necessidade de validação padronizada por fase |
...
| **Correcao** | Implementado: plan.created event, RULE-020, plan md prepare, 4 pipelines, template. Futuro: shugo pipeline run/exec/notify/list/status, backbone sync. |

### NEW-001 Runtime validation shitenno/profile/
...
| **Owner** | unassigned |
| **Data** | 2026-07-06 |
| **Fonte** | Feedback de sessao — erro cryptico se shugo init nao foi feito |
| **Modulos** | src/scorer.ts, src/scaffolder.ts |
| **Descricao** | Nao existe validacao em runtime para existencia de `shitenno/profile/` antes de tentar ler o ficheiro de config. Se o projecto ainda nao fez `shugo init`, o utilizador ve erro críptico em vez de mensagem amigavel. |
...
| **Correcao** | Criar `src/tier-gate.ts`. Modificar `bin/shugo.ts` para verificar tier antes de executar comandos restritos. |
...
| **Correcao** | Criar plugin OpenCode que chama `shugo briefing --summary` antes de cada tarefa e injeta no system prompt. |
...
| **Owner** | unassigned |
| **Data** | 2026-07-11 |
| **Fonte** | Observação durante planeamento do Shugo Living |
...
| **Correcao** | Criar comando `shugo plan sync` que: (1) lê checkboxes do plano, (2) cria/actualiza items correspondentes no BACKLOG.md, (3) sincroniza estados (`[x]` → `Done`, `[ ]` → `Backlog`). Considerar webhook ou watcher para actualização em tempo real. |
...
| **Correcao** | Criar `shugo detect --approve RULE-001` e `shugo detect --reject RULE-001`. |
...
| **Descricao** | O `shugo feedback` so regista outcome do agente. O utilizador nao pode avaliar a sessao. |
...
### 2.3 shugo bench --compare historico
...
| **Correcao** | Salvar resultado em `shitenno/reports/bench-YYYY-MM-DD.json`. Adicionar `--compare`. |

### 2.4 shugo feedback --list
...
| **Correcao** | Adicionar campo `trialEndsAt` no `shitenno/config.json`. |
...
| **Correcao** | Criar extensao VS Code que le `.shugo/BRIEFING.md`. |
...
| **Correcao** | Criar `shugo hooks --install` que configura git hooks via `core.hooksPath`. |
...
| **Correcao** | Dashboard `shugo analytics` que le `usage.jsonl`. |
...
| **Owner** | unassigned |
| **Data** | 2026-06-30 |
| **Fonte** | shugo doctor --json |
| **Modulos** | shitenno/ |
| **Descricao** | Knowledge graph nao inicializado. |
| **Correcao** | Executar `shugo audit` para popular knowledge graph. |
...
| 3.6 | shugo dashboard --live | Baixo | Watch mode com atualizacao periodica |
...
| 3.10 | shugo --quiet / --no-color | Baixo | Modo scriptavel para CI/CD |
| 3.11 | shugo init --dry-run | Baixo | Preview do que seria criado sem criar |
| 3.12 | shugo upgrade --dry-run | Baixo | Preview do que seria instalado sem instalar |
| 3.13 | shugo audit --fix | Medio | Auto-fix para problemas de governance detectados |
...
| 3.19 | shugo detect --approve/--reject | Medio | Aprovar ou rejeitar regras candidatas do pattern-detector |
| 3.20 | shugo feedback --outcome failure auto-link | Baixo | Ao reportar falha, sugerir areas baseado no pattern-detector |
...
| 3.25 | shugo bench --save / --load | Baixo | Salvar/carregar benchmarks para comparacao offline |
| 3.26 | shugo status --fix | Baixo | Auto-fix para problemas de governance (como doctor) |
...
| 3.31 | shugo detect --format markdown | Baixo | Saida markdown para detect (atualmente so text/json) |
...
| 3.34 | shugo dashboard --export csv | Baixo | Exportar dados do dashboard em CSV |
| 3.35 | Plugin sandboxing | Baixo | Isolar plugins em workers para seguranca |
| 3.36 | Decidir nome do produto | Baixo | Shugo Gaude, Prism, Codex, ou outro |
...
| 3.47 | D3 Migration guide | Baixo | Como migrar de outros tools para Shugo |
...
| **P1** | 23 | Arquitetura, docs, seguranca, monetizacao, AI integration, Shugo Living (fases 2-3, memoria, pipeline, pipelines de validação) |
...
| 2.1 | Medio | shugo detect --approve/--reject |
| 2.3 | Baixo | shugo bench --compare |
| 2.4 | Baixo | shugo feedback --list |
...
### BACKLOG-2026_07_02_SHITENNO_DASHBOARD_RESTRUCTURE — Shitenno Dashboard — Plano de Refactoracao por Camadas de Conhecimento
...
| **Descricao** | Shitenno Dashboard — Plano de Refactoracao por Camadas de Conhecimento |
| **Correcao** | Verificar checklist no plano `governance/plans/2026-07-02-shitenno-dashboard-restructure.md` |


### BACKLOG-2026_07_11_SHITENNO_LIVING_PLANO_V2_3FASES — Shitenno Living — Plano de Implementação (v2 — Roteiro em 3 Camadas)
...
| **Fonte** | shugo plan md prepare |
| **Modulos** | governance/plans/ |
| **Descricao** | Shitenno Living — Plano de Implementação (v2 — Roteiro em 3 Camadas) |
| **Correcao** | Verificar checklist no plano `governance/plans/2026-07-11-shitenno-living-plano-v2-3fases.md` |
...
### BACKLOG-2026_07_12_PLANO_CORRECAO_WATCH_LOOP_E_FASE2 — Plano de Correção — Loop do `shugo watch` + Finalizar Fase 2 (LIVING-002)
...
| **Fonte** | shugo plan md prepare |
| **Modulos** | governance/plans/ |
| **Descricao** | Plano de Correção — Loop do `shugo watch` + Finalizar Fase 2 (LIVING-002) |
...
- [ ] Teste manual: `shugo watch` rodando, editar um plano real com conteúdo que dispara sync — confirmar
- [ ] `installReactiveHooks()` implementado, testado (5 testes acima passando).
- [ ] `shugo init` chama o instalador automaticamente — testar em projeto novo: `.git/hooks/post-commit`
...
- [ ] Atualizar `shitenno/docs/BACKLOG.md` — mudar `LIVING-002` de `Status: Backlog` para
- [ ] Os 2 testes novos acima passam.
- [ ] `pnpm test` completo continua em verde (sem regressão nos testes existentes de
- [ ] Teste manual: `shugo watch` rodando, editar um plano real com conteúdo que dispara sync — confirmar
- [ ] `installReactiveHooks()` implementado, testado (5 testes acima passando).
...
### BACKLOG-2026_07_13_PLANO_BUG002_ENTROPIA — Plano Consolidado — Shugo Living: BUG-002 (fim-a-fim), Redesenho da Entropia, Item Futuro
...
| **Fonte** | shugo plan md prepare |
| **Modulos** | governance/plans/ |
| **Descricao** | Plano Consolidado — Shugo Living: BUG-002 (fim-a-fim), Redesenho da Entropia, Item Futuro |
...
### BACKLOG-PLANO_MONETIZACAO_SHITENNO — Plano de Monetização — shitenno (Free + Token Mensal)
...
| **Fonte** | shugo plan md prepare |
| **Modulos** | governance/plans/ |
| **Descricao** | Plano de Monetização — shitenno (Free + Token Mensal) |
| **Correcao** | Verificar checklist no plano `governance/plans/PLANO-MONETIZACAO-SHITENNO.md` |
...
- [ ] `shugo license activate <key>` testado ponta a ponta em modo teste
- [ ] `PUBLIC_JWK` real (não o placeholder) commitado em `src/license.ts`
- [ ] `PRIVATE_JWK` **fora** do repositório (só no Cloudflare, via `wrangler secret`)
- [ ] Worker deployado e testado manualmente (`curl` no `/refresh` com uma key de teste)
- [ ] Payment Link do Stripe testado em modo teste (Stripe tem test mode com cartão `4242 4242 4242 4242`)
...
### BACKLOG-PLAN_MONETTIZATION_IMPLEMENTATION — Plano de Implementação — Monetização Shugo CLI
...
| **Fonte** | shugo plan md prepare |
| **Modulos** | governance/plans/ |
| **Descricao** | Plano de Implementação — Monetização Shugo CLI |
...
### BACKLOG-HANDOFF_AGENTE — Handoff para o agente — shitenno
...
| **Fonte** | shugo plan md prepare |
| **Modulos** | governance/plans/ |
| **Descricao** | Handoff para o agente — shitenno |
| **Correcao** | Verificar checklist no plano `governance/plans/handoff-agente.md` |


### BACKLOG-PLANO_DAEMON_KERNEL — Plano: transformar o daemon no kernel do shitenno
...
| **Fonte** | shugo plan md prepare |
| **Modulos** | governance/plans/ |
| **Descricao** | Plano: transformar o daemon no kernel do shitenno |
...
| **Owner** | executor |
| **Data** | 2026-07-16 |
| **Fonte** | shugo audit enterprise (score 59/100) |
| **Modulos** | governance/plans/ |
...
#### Passos do Plano
- [ ] `shugo audit --level enterprise` → score >80
- [ ] 0 critical issues
- [ ] <30 warnings
- [ ] `pnpm run lint` → 0 erros
- [ ] `pnpm run typecheck` → 0 erros
...
### BACKLOG-2026_07_16_AUDIT_CONFIDENCE_SUPPRESSION1 — PLAN-AUDIT-CONF — Confidence Score + Suppression Baseline no `shugo audit`
...
| **Fonte** | shugo plan md prepare |
| **Modulos** | governance/plans/ |
| **Descricao** | PLAN-AUDIT-CONF — Confidence Score + Suppression Baseline no `shugo audit` |
...
- [ ] Passo 6 — Comando `shugo audit suppress` e flags de CLI
...
- [ ] Rodar `shugo audit --level enterprise` num repo real e comparar `healthScore` antes/depois — divergência deve ser explicável (não aleatória).
- [ ] Suprimir um issue via `shugo audit suppress <hash>` e confirmar que ele some do próximo `shugo audit` mas aparece em `shugo audit --show-suppressed`.
- [ ] `npm run typecheck` sem erros após a mudança.
- [ ] `health-auditor.test.ts` e `suggestion-engine` tests continuam passando (rodar com fixtures antigas para garantir que `confidence` default não quebra contrato existente).
- [ ] Rodar `shugo audit --level enterprise` num repo real e comparar `healthScore` antes/depois — divergência deve ser explicável (não aleatória).
- [ ] Suprimir um issue via `shugo audit suppress <hash>` e confirmar que ele some do próximo `shugo audit` mas aparece em `shugo audit --show-suppressed`.
...
#### Passos do Plano
- [ ] `shugo audit --apply` num repo de teste com um `unused_import` conhecido: import é removido, `npm run typecheck` roda automaticamente, e se passar o fix fica; se um fix quebrar o build, o arquivo volta ao estado original e o issue é reportado como "fix tentado e revertido".
- [ ] `shugo audit --changed` num branch com 2 arquivos alterados de um projeto com 200 arquivos: tempo de execução cai proporcionalmente (medir com `durationMs` já existente no `HealthAuditReport`).
- [ ] Output humano do `shugo audit` mostra um card por dimensão com nota/score antes da lista de issues.
- [ ] `shugo audit --apply` num repo de teste com um `unused_import` conhecido: import é removido, `npm run typecheck` roda automaticamente, e se passar o fix fica; se um fix quebrar o build, o arquivo volta ao estado original e o issue é reportado como "fix tentado e revertido".
- [ ] `shugo audit --changed` num branch com 2 arquivos alterados de um projeto com 200 arquivos: tempo de execução cai proporcionalmente (medir com `durationMs` já existente no `HealthAuditReport`).
- [ ] Output humano do `shugo audit` mostra um card por dimensão com nota/score antes da lista de issues.
...
#### Passos do Plano
- [ ] Rodar o daemon com `SHITENNO_DAEMON_LOG_MAX_BYTES=1024` (valor baixo, só para teste) e gerar >1KB de log — confirmar que `daemon.log.1` aparece e `daemon.log` reinicia vazio.
- [ ] Gerar rotações suficientes para exceder `SHITENNO_DAEMON_LOG_MAX_FILES` (ex. =2) e confirmar que nunca existem mais que 2 arquivos `.log.N` simultâneos.
...
- [ ] `shugo daemon logs` rodando durante uma rotação continua funcionando sem crashar e sem duplicar linhas.
- [ ] `npm run typecheck` e `npm test` passam sem quebrar nada em `daemon.ts` ou seus testes relacionados.
- [ ] Documentar as duas env vars novas (`SHITENNO_DAEMON_LOG_MAX_BYTES`, `SHITENNO_DAEMON_LOG_MAX_FILES`) em `docs/reference/cli.md` ou onde as outras env vars do daemon (`SHITENNO_DAEMON_LOG`, `SHITENNO_NO_DAEMON`) já estão documentadas.
- [ ] Rodar o daemon com `SHITENNO_DAEMON_LOG_MAX_BYTES=1024` (valor baixo, só para teste) e gerar >1KB de log — confirmar que `daemon.log.1` aparece e `daemon.log` reinicia vazio.
- [ ] Gerar rotações suficientes para exceder `SHITENNO_DAEMON_LOG_MAX_FILES` (ex. =2) e confirmar que nunca existem mais que 2 arquivos `.log.N` simultâneos.
- [ ] Reiniciar o daemon (`daemon stop` + `daemon start`) com um `daemon.log` já grande (>maxBytes) e confirmar que a primeira escrita pós-restart já dispara rotação corretamente (valida o `initLogByteCounter`).
- [ ] `shugo daemon logs` rodando durante uma rotação continua funcionando sem crashar e sem duplicar linhas.
...
### BACKLOG-PLAN_2026_07_16_SYSTEM_RESILIENCE_REVISADO — Plano de Ação (Revisado) — Resiliência do Sistema Shitenno
...
| **Fonte** | shugo plan md prepare |
| **Modulos** | governance/plans/ |
| **Descricao** | Plano de Ação (Revisado) — Resiliência do Sistema Shitenno |
...
### BACKLOG-PLAN_2026_07_17_CONSOLIDACAO_PRODUTIZACAO_MULTI_PROJETO — PLAN-2026-07-17 — Consolidação Arquitetural para Produtização Multi-Projeto
...
| **Fonte** | shugo plan md prepare |
| **Modulos** | governance/plans/ |
| **Descricao** | PLAN-2026-07-17 — Consolidação Arquitetural para Produtização Multi-Projeto |
| **Correcao** | Verificar checklist no plano `governance/plans/PLAN-2026-07-17-consolidacao-produtizacao-multi-projeto.md` |
...
### BACKLOG-PLAN_2026_07_17_GOVERNANCA_REGRAS_MANIFESTO_ENFORCEMENT — PLAN-2026-07-17 — Governança de Regras: Rule Manifest + Injeção Posicional + Enforcement Mecânico
...
| **Fonte** | shugo plan md prepare |
| **Modulos** | governance/plans/ |
| **Descricao** | PLAN-2026-07-17 — Governança de Regras: Rule Manifest + Injeção Posicional + Enforcement Mecânico |
| **Correcao** | Verificar checklist no plano `governance/plans/PLAN-2026-07-17-governanca-regras-manifesto-enforcement.md` |
