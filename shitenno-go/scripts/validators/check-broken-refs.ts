...
import { SHITEN, README, SYSTEM_MAP, DOCS, type ValidatorContext, pass, warn } from "./shared.js";
...
export function checkBrokenReferences(ctx: ValidatorContext) {
  ...
  for (const filePath of filesToCheck) {
    ...
    for (const ref of uniquePaths) {
      ...
      const fullPath = resolve(SHITEN, ref);
      if (!existsSync(fullPath)) {
        const parentRef = ref.split("/").slice(0, -1).join("/");
        const parentPath = resolve(SHITEN, parentRef);
