...
import { SHITEN, PACKAGE_JSON, type ValidatorContext, pass, warn } from "./shared.js";

export function checkScriptReferences(ctx: ValidatorContext) {
  console.log("\n📜 Checking script references in WORKFLOW.md...\n");

  const workflowPath = resolve(SHITEN, "governance", "WORKFLOW.md");
  ...
  for (const script of uniqueScripts) {
    if (script.startsWith("shiten ")) continue;
