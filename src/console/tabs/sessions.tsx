...
export function SessionsTab({ data, scrollOffset = 0, expandedItem, onExpandItem }: SessionsTabProps): React.ReactElement {
  ...
  const isMetricsExpanded = expandedItem === "session-metrics";
    ...
    <Box flexDirection="column" gap={1}>
      ...
      <SectionBox title="Session Metrics">
        <Box
          ...
          borderStyle={isMetricsExpanded ? "bold" : undefined}
          borderColor={isMetricsExpanded ? "cyan" : undefined}
          paddingX={isMetricsExpanded ? 1 : 0}
        >
          <Box>
            <Text color="cyan">{isMetricsExpanded ? "▼" : "▶"} </Text>
