 ...
 *
 * Determines whether a file change in nexus-system/ is significant enough
...
export function detectDirectoryScore(filePath: string, nexusDir: string): number {
  const relative = filePath.slice(nexusDir.length + 1);
...
export function calculateSignificance(
  ...
  // 2. Directory score
  const directoryScore = detectDirectoryScore(filePath, nexusDir);
