...
import { SHITEN_DIR_NAME } from "../src/constants.js";
...
if (isInitialized) {
  ...
  const docEngine = new DocEngine(shitenDir);
  ...
  if (!process.env.SHITEN_CHILD) {
    ...
    initPlanBacklogSync(projectRoot, shitenDir);

    showBriefingSummary(projectRoot, shitenDir);
  }
}
 ...
 */
function showBriefingSummary(projectRoot: string, shitenDir: string): void {
  try {
...
program.configureHelp({
  ...
  formatHelp(_cmd, _helper) {
    let output = "";
    output += `${chalk.bold.cyan("shiten")} ${chalk.gray("— AI governance ecosystem")}\n\n`;

    // Usage
    output += `${chalk.bold("Usage:")}\n`;
    output += `  shiten <command> [options]\n\n`;

    output += `${chalk.bold("Quick Start:")}\n`;
    output += `  shiten --help            ${chalk.gray("Show all commands")}\n`;
    output += `  shiten --help <command>  ${chalk.gray("Show help for a command")}\n`;
    output += `  shiten --version         ${chalk.gray("Show version")}\n\n`;
...
const helpCmd = new Command("help")
  ...
  .action((cmdName: string | undefined) => {
    ...
    if (!cmd) {
      ...
      console.log(chalk.gray("  Run 'shiten --help' to see all available commands."));
      process.exitCode = 1;
      return;
    }

    console.log("");
    console.log(`${chalk.bold.cyan(`shiten ${cmd.name}`)} — ${cmd.description}`);
    console.log("");
...
if (isInitialized && currentSessionId) {
  ...
  endSession(shitenDir, currentSessionId);
  clearSessionContext();
  if (!process.env.SHITEN_CHILD) {
    stopWatching();
  }
