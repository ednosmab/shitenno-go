# Plano: nexus init + audit para projectos activos

## Objectivo
Quando `nexus init` detecta que o projecto já está inicializado, o sistema deve:
...
- No branch de projecto existente (linha 127-196):
  ...
  - **Se starter:** mostrar sugestão "Quando tiveres código, corre `nexus audit`"
...
- Ficheiro: `src/commands/init.ts`
- Após `displayCapabilities(profile)`:
  - Se `isStarterProject(analysis)` → mostrar "Run `nexus audit` when ready"
