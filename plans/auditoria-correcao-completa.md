# Plano de Correção — Auditoria Completa nexus-cli
...
```typescript
// ── Substituir isNexusPlugin (linhas 165-173) ──
...
```typescript
/**
 * logger.ts — Logging centralizado para Nexus
 *
...
```typescript
/**
 * constants.ts — Constantes compartilhadas do Nexus
 *
...
**Arquivos:** `scorer.ts`, `health-auditor.ts`, `pattern-detector.ts`, `shared.ts`, `nexus-state-machine.ts`
...
```typescript
/**
 * errors.ts — Erros tipados do Nexus
 *
...
### 2.7 Adicionar evento faltante ao NexusEventType
...
**Objetivo:** Atualizar README para refletir o estado atual do Nexus System (12 comandos, features completas).
...
9. **Configuração** — opencode.json e nexus-profile
...
nexus upgrade --level pleno      # atualizar para L2
nexus upgrade --level senior     # atualizar para L3
